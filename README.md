...
- **領域マッピング** - 4点コーナーで囲んだ領域内の座標を0-1に正規化して送信（斜め設置のカメラにも対応）。矩形の自動検出でコーナーを一括設定することも可能
...
コーナーはドラッグで調整できるほか、**Detect** ボタンで映像内の矩形から自動設定できます。押したあとプレビューの領域内を1回クリックすると、そこを含む矩形を探します（Escキーでクリックを省略して全自動）。背景がシンプルなほど精度が上がるため、あくまで初期位置を決める補助として使い、必要に応じて手で微調整してください。「Show what Detect saw」をオンにすると、検出したエッジ・直線・候補が重ねて表示されます。
